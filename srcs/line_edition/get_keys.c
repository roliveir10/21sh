...
{
	if (ft_isaltc(str) && !env->select)
		ft_init_cpy(env);
	else if (ft_isaltv(str) && !env->select)
...
static int			ft_line_history(t_env *env, char *str)
{
...
int					ft_line_arrow(t_env *env, char *str)
{
	...
	else if (!ft_strcmp(str, RJUMP))
		ft_rjump(env);
	else if (!ft_strcmp(str, LJUMP))
		ft_ljump(env);
	else if (!ft_strcmp(str, UJUMP))
		ft_cursor_motion(env, MLEFT, env->cm->term_x);
	else if (!ft_strcmp(str, DJUMP))
		ft_cursor_motion(env, MRIGHT, env->cm->term_x);
	else if (!ft_strcmp(env->tc->kh, str))
		ft_home(env);
	else if (!ft_strcmp(TEND, str))
		ft_end(env);
	else
		return (0);
	return (1);
...
static int			ft_line_ascii(t_env *env, char *str, int ret)
{
	...
	else if (!ft_strcmp(env->tc->kd, str))
